// -*- C++ -*-
//
// Package:    CheckTauSigCands
// Class:      CheckTauSigCands
// 
/**\class CheckTauSigCands CheckTauSigCands.cc BoostedTauAnalysis/CheckTauSigCands/src/CheckTauSigCands.cc
...
class CheckTauSigCands : public edm::EDAnalyzer {
   public:
      explicit CheckTauSigCands(const edm::ParameterSet&);
      ~CheckTauSigCands();
...
//
CheckTauSigCands::CheckTauSigCands(const edm::ParameterSet& iConfig)
...
CheckTauSigCands::~CheckTauSigCands()
{
...
void
CheckTauSigCands::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{  
...
/////////////////////////////////////////////////////////////////////////
  ...
  /* Loop through matched taus to study sig/iso candidate properties */
   ...
   for(PFTauCollection::const_iterator iTau = HPSPFTaus->begin(); iTau != HPSPFTaus->end(); ++iTau)
	 ...
	 PFCandidateRefVector IsoCands = iTau->signalPFCands();
	 //PFCandidateRefVector IsoCands = (*iTau)->signalPFCands(); // after you've matched to TMTH
	 ...
	 for(PFCandidateRefVector::const_iterator cand = IsoCands.begin(); cand != IsoCands.end(); ++cand)
	  ...
	  if(Id == PFCandidate::h)
		...
		{
		  cout << "there are gsf tracks" << endl;
		...
		{ cout << "there are no gsftracks, strangely" << endl; }
	      ...
	      unsigned int genEleVectorSize = genElesZee.size();
	      if(genEleVectorSize!=0)
		  ...
		  if (mindelR < 0.3)
		    {
		      ...
		      // cout << "mother of e = " << ()->mother()->pdgId();
		      // cout << "grandmother of e = " << (*iEle)->mother()->pdgId();
		      //cout << "great-grandmother of e = " << (*iEle)->mother()->pdgId();
...
void 
CheckTauSigCands::beginJob()
{
  cout << "Beginning job" << endl;
  out_ = new TFile(outFileName_.c_str(), "RECREATE");

  tmthDR = new TH1D("tmthDR", "deltaR of gen tau_mu and gen tau_had from a decay", 50., 0., 5.);
  IsoCandFrequency = new TH1D("IsoCandFrequency", "Frequency of particle IDs among the signal PF Candidates of the taus", 10., 0., 10.);
  ...
  pfElesPizero_pT = new TH1D("pfElesPizero_pT", "pT of PF e matched to gen e from pizero decay", 100., 0., 200.);
  ...
  pfMu_ZtmMatch_pT = new TH1D("pfMu_ZtmMatch_pT", "pT of PF mus matched to Wtm gen mus", 100., 0., 200.);
  pfMu_ZmMatch_pT = new TH1D("pfMu_ZmMatch_pT", "pT of PF mus matched to Wm gen mus", 100., 0., 200.);
...
CheckTauSigCands::endJob() 
{
  ...
  TCanvas *candpT = new TCanvas("candpT", "pT's of different kinds of signal PF candidates", 750, 750);
  candpT->cd();
  //X_pT->Write();
  if(h_pT->Integral()!=0.)
  ...
  out_->cd();
  candpT->Write();
  TCanvas *candET = new TCanvas("candET", "ET's of different kinds of signal PF candidates", 750, 750);
  candET->cd();
  if(h_ET->Integral()!=0.)
  ...
  //hHF_dz->Write();
  //egammaHF_dz->Write();
  TCanvas *pfMu_pT = new TCanvas("pfMu_pT", "pT distributions of PF muon signal candidates matched to gen muons", 750, 750);
  pfMu_pT->cd();
...
void 
CheckTauSigCands::beginRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a run  ------------
void 
CheckTauSigCands::endRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when starting to processes a luminosity block  ------------
void 
CheckTauSigCands::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
void 
CheckTauSigCands::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
CheckTauSigCands::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(CheckTauSigCands);
